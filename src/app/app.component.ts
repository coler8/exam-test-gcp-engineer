...
export class AppComponent {
  ...
  allQuestions = [
    ...
    },
    {
      ...
      answers: [
        ...
        "Create a secondary index using the following Data Definition Language (DDL): (Imagen 2)",
      ],
      correctAnswer: 2,
    },
    {
    ...
    },
    {
      ...
      question:
        "Your company has an internal application for managing transactional orders. The application is used exclusively by employees in a single physical location. The application requires strong consistency, fast queries, and ACID guarantees for multi-table transactional updates. The first version of the application is implemented in PostgreSQL, and you want to deploy it to the cloud with minimal code changes. Which database is most appropriate for this application?",
      answers: ["Bigtable", "BigQuery", "Cloud SQL", "Firestore"],
      correctAnswer: 2,
    },
    {
      id: 218,
  ...
  showQuestion(): void {
    ...
    this.shuffledAnswers = this.shuffleArray(
      this.currentQuestion.answers.map((text: string, i: number) => ({
        text,
        isCorrect: i === this.currentQuestion.correctAnswer,
      }))
    );
    this.answered = false;
  ...
  selectAnswer2(questionIndex: number, answerIndex: number): void {
  ...
  getShuffledQuestions(): any[] {
    if (this.mode === 'random') {
      return this.shuffleArray([...this.allQuestions]).slice(0, 50);
    } else {
      const startIndex = this.blockIndex * 50;
      const endIndex = Math.min(startIndex + 50, this.allQuestions.length);
      return this.shuffleArray(this.allQuestions.slice(startIndex, endIndex));
    }
  }
